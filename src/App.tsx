  ...
  const paths = {
    note: (
      <>
        ...
        <path d="M15 3v4h4M8 11h8M8 15h8" />
      </>
    ),
    paper: (
      <>
...
const paperProgress = (paper: Paper) =>
  paper.stages.reduce(
    (sum, stage) =>
      sum + (stage.tasks.every((task) => task.done) ? stage.score : 0),
    0,
  );
...
export function App() {
    ...
    const syncDatabase = async () => {
      try {
        ...
        if (
          ...
          remoteState.notes.length ||
          remoteState.papers.length
        ) {
          if (mounted) {
          ...
          // A newly authenticated user may have no rows yet. Migrate the
          // user's local workspace once, then replace legacy IDs such as
          // "career" with the UUIDs generated by PostgreSQL migration.
          const userState = loadState(oauthUser.id);
          const localState = userState.notebooks.length || userState.notes.length || userState.papers.length
            ? userState
            : loadState("anonymous");
        ...
        if (
          ...
          setSyncNotice(`数据库同步暂不可用：${error.message}`);
      }
    };
    void syncDatabase();
    return () => {
  ...
  const paper =
    ...
    state.papers.filter((item) => item.notebookId === notebookId).at(-1);
  ...
  const reportSyncFailure = (error: unknown) =>
    setSyncNotice(
      `数据库同步失败，本地数据已保留：${error instanceof Error ? error.message : "未知错误"}`,
    );

  const createNotebook = async (book: {
    id: string;
    name: string;
    color: string;
  }) => {
    ...
    try {
      ...
      update((current) => ({
        ...
        notebooks: current.notebooks.map((item) =>
          item.id === book.id ? remote : item,
        ),
        notes: current.notes.map((note) =>
          note.notebookId === book.id
            ? { ...note, notebookId: remote.id }
            : note,
        ),
        papers: current.papers.map((paper) =>
          paper.notebookId === book.id
            ? { ...paper, notebookId: remote.id }
            : paper,
        ),
      }));
  ...
  const saveNote = () => {
    ...
    const existing = editor.id
      ? state.notes.find((note) => note.id === editor.id)
      : undefined;
    const next = existing
      ? { ...existing, tag: editor.tag, color: editor.color, body }
      : {
          id: id(),
          notebookId,
          tag: editor.tag,
        ...
        void createNoteRemote(next)
          .then((remote) =>
            update((current) => ({
              ...current,
              notes: current.notes.map((note) =>
                note.id === next.id ? { ...remote, source: next.source } : note,
              ),
            })),
          )
          .catch(reportSyncFailure);
      ...
      void createNoteRemote(next)
        .then((remote) =>
          update((current) => ({
            ...current,
            notes: current.notes.map((note) =>
              note.id === next.id ? { ...remote, source } : note,
            ),
          })),
        )
        .catch(reportSyncFailure);
  ...
  const submit = async () => {
    ...
    try {
      const version =
        state.papers.filter((item) => item.notebookId === notebookId).length +
        1;
      let next: Paper;
      try {
        ...
        setGenerationNotice(
          `知乎直答暂不可用，已使用本地方案生成：${error instanceof Error ? error.message : "未知错误"}`,
        );
      }
    ...
    <div className="app-shell">
      ...
      {syncNotice && (
        <div className="generation-notice sync-notice">
          {syncNotice}
          <button onClick={() => setSyncNotice(null)}>×</button>
        </div>
      )}
      {page === "notes" ? (
        <NotesPage
          ...
          onNotebook={setNotebookId}
          onState={update}
          onEditor={setEditor}
          onSave={saveNote}
          onExcerpt={() => setExcerptOpen(true)}
          onSubmit={() => setConfirmingSubmit(true)}
          onCreateNotebook={createNotebook}
          onDeleteNotebook={(id) => {
            if (databaseReady)
              void deleteNotebookRemote(id).catch(reportSyncFailure);
          }}
        ...
        <PapersPage
          ...
          onRegenerate={() => setConfirmingSubmit(true)}
          onDeletePaper={(id) => {
            if (databaseReady)
              void deletePaperRemote(id).catch(reportSyncFailure);
          }}
          onToggleTask={(id, done) => {
            if (databaseReady)
              void updateTaskRemote(id, done).catch(reportSyncFailure);
          }}
        />
      )}
      {excerptOpen && (
      ...
      {generating && (
        <div className="generating-overlay" role="status" aria-live="assertive">
          <div className="generating-card">
            <span className="large-spinner" />
            <h2>正在整理你的人生答卷</h2>
            <p>知乎直答正在阅读所选材料并生成行动方案，通常需要 10–30 秒。</p>
            <small>请保持页面开启，完成后会自动进入答卷页。</small>
          </div>
        </div>
      )}
    </div>
...
function SubmitConfirm({
  ...
  return (
    ...
    >
      ...
      >
        <span className="confirm-seal">卷</span>
        <h2 id="submit-title">提交这 {count} 条材料？</h2>
        <p>
          所选笔记正文及其摘录来源将发送给知乎直答，用于生成结构化行动答卷。请确认其中不包含你不希望传输的隐私信息。
        </p>
        <div>
          <button onClick={onCancel}>返回检查</button>
          <button className="confirm-primary" onClick={onConfirm}>
            确认并生成
          </button>
        </div>
      </section>
  ...
  return (
    <header className="appbar">
      <button className="brand" onClick={() => onPage("notes")}>
        ...
        <span>平时做笔记，关键时提交答卷</span>
      </button>
      <nav className="switcher" aria-label="主导航">
        <button
          className={page === "notes" ? "active" : ""}
          onClick={() => onPage("notes")}
        >
          <Icon name="note" />
          笔记
        </button>
        <button
          className={page === "papers" ? "active" : ""}
          onClick={() => onPage("papers")}
        >
          <Icon name="paper" />
          答卷
        </button>
...
interface NotesProps {
  ...
  onState: (fn: (state: AppState) => AppState) => void;
  onEditor: (editor: EditorState) => void;
  onSave: () => void;
  onExcerpt: () => void;
  onSubmit: () => void;
  onCreateNotebook: (book: { id: string; name: string; color: string }) => void;
...
function NotesPage(props: NotesProps) {
  ...
  const addBook = () => {
    ...
    const next = {
      id: id(),
      name: bookName.trim(),
      color: ["#c8d3e0", "#dec8d3", "#c4d4bf", "#e5d1ac"][
        state.notebooks.length % 4
      ],
    };
    onState((current) => ({
      ...current,
      notebooks: [...current.notebooks, next],
    }));
  ...
  const deletingBook = state.notebooks.find(
    (book) => book.id === deletingBookId,
  );
  const deletingNoteCount = state.notes.filter(
    (note) => note.notebookId === deletingBookId,
  ).length;
  const deletingPaperCount = state.papers.filter(
    (paper) => paper.notebookId === deletingBookId,
  ).length;
  const deleteBook = () => {
    ...
    <main className="notes-page">
      <section className="shelf" aria-label="笔记本">
        {state.notebooks.map((book) => (
          <div
            key={book.id}
            className={`book ${book.id === notebookId ? "active" : ""}`}
          >
            <button className="book-select" onClick={() => onNotebook(book.id)}>
              <i style={{ background: book.color }} />
              <span>{book.name}</span>
              <small>
                {
                  state.notes.filter((note) => note.notebookId === book.id)
                    .length
                }
              </small>
            </button>
        ...
        <div className="notes-inner">
          <div className="quick-actions">
            <button
              ...
              onClick={() =>
                onEditor({ tag: "idea", color: NOTE_COLORS[0], body: "" })
              }
            >
              <Icon name="plus" />
              新建笔记
            </button>
            <button className="tool" onClick={onExcerpt}>
          ...
          {editor && (
            <NoteEditor
              ...
              onChange={onEditor}
              onSave={onSave}
              onCancel={() => onEditor(null)}
            />
          )}
          {!notes.length && !editor ? (
            <div className="empty">
              <span className="empty-seal">空</span>
              <h1>这一册还没有笔记</h1>
              <p>写下一个困扰、一项限制，或摘录一句让你停下来的话。</p>
            </div>
          ) : (
            notes.map((note) => (
              <NoteCard
      ...
      <footer className="dock">
        <div className="dock-inner">
          <div>
            <span className={`count ${selectedCount ? "hot" : ""}`}>
              {generating ? "正在作答" : `已选 ${selectedCount}`}
            </span>
            <p aria-live="polite">
              {generating
                ? "正在调用知乎直答整理材料，通常需要 10–30 秒，请不要关闭页面。"
                : selectedCount
                  ? "这些材料已装订，可以提交一份新答卷。"
                  : "勾选笔记，把它们投进答卷。"}
            </p>
          </div>
          <div className={`submit-wrap ${selectedCount ? "armed" : ""}`}>
            <span className="seal-ring" />
            <button
              aria-label={
                generating ? "正在生成答卷" : `提交 ${selectedCount} 条笔记`
              }
              className={`submit-seal ${selectedCount ? "ready" : ""} ${generating ? "busy" : ""}`}
              disabled={!selectedCount || generating}
              onClick={onSubmit}
            >
              {generating ? <span className="spinner" /> : "提交"}
            </button>
          </div>
        </div>
      </footer>
      {deletingBook && (
        <div
          className="overlay confirm-overlay"
          onMouseDown={(event) =>
            event.target === event.currentTarget && setDeletingBookId(null)
          }
        >
          <section
            className="submit-confirm"
          ...
          >
            <span className="confirm-seal">删</span>
            <h2 id="delete-book-title">删除“{deletingBook.name}”笔记本？</h2>
            <p>
              此操作会同时删除其中的 {deletingNoteCount} 条笔记和{" "}
              {deletingPaperCount} 份答卷，且无法恢复。
            </p>
            <div>
              <button onClick={() => setDeletingBookId(null)}>取消</button>
              <button className="danger-primary" onClick={deleteBook}>
                确认删除
              </button>
            </div>
          </section>
  ...
  return (
    <section className="note-editor">
      ...
      <div className="tag-options">
        {(["idea", "limit", "try"] as const).map((tag) => (
          <button
            key={tag}
            className={editor.tag === tag ? "active" : ""}
            onClick={() => onChange({ ...editor, tag })}
          >
            <i style={{ background: TAGS[tag].color }} />
            {TAGS[tag].label}
          </button>
        ))}
      </div>
      <span className="eyebrow color-label">标签颜色</span>
      <div className="color-options" role="radiogroup" aria-label="标签颜色">
        {NOTE_COLORS.map((color) => (
          <button
            ...
            className={editor.color === color ? "active" : ""}
...
function NoteCard({
  ...
  return (
    <article className={`note-card ${note.selected ? "selected" : ""}`}>
      <label className="check">
        <input type="checkbox" checked={note.selected} onChange={onToggle} />
        <span />
      </label>
      <div className="note-main">
        <div className="note-meta">
          <span className="tag" style={{ background: note.color ?? tag.color }}>
            <i />
            {tag.label}
          </span>
          ...
          <div className="card-actions">
            {note.tag !== "quote" && (
              <button aria-label="编辑" onClick={onEdit}>
                <Icon name="edit" />
              </button>
            )}
            <button aria-label="删除" onClick={() => setDeleting(true)}>
              <Icon name="trash" />
            </button>
          </div>
        </div>
        ...
        {note.source && (
          ...
          >
            <span>知乎摘录</span>
            {note.source.author}：《{note.source.title}》
          </a>
        )}
        {deleting && (
...
function ExcerptModal({
  ...
  onClose: () => void;
  onAdd: (body: string, source: Source) => void;
}) {
  ...
  const runSearch = async (refresh = false) => {
    ...
    setError("");
    try {
      setItems(uniqueSearchItems(await searchZhihu(query.trim(), refresh)));
    } catch (reason) {
      setItems([]);
  ...
  const chooseSelection = () => {
    ...
    const selectedSentences = Array.from(
      document.querySelectorAll<HTMLElement>("[data-quote-sentence]"),
    )
      .filter((element) => {
        try {
  ...
  return (
    ...
    >
      <section className="modal">
        <header>
          <div>
            <span className="eyebrow">知乎经验摘录</span>
            <h2>{chosen ? "划出最想留下的一句" : "找一条真实经验"}</h2>
          </div>
        ...
        {!chosen ? (
          <>
            ...
            {!loading && !error && !items.length && (
              <p className="api-empty">没有找到相关内容，请换个关键词。</p>
            )}
            <div className="results">
              {items.map((item) => (
                <button key={item.id} onClick={() => setChosen(item)}>
                  <strong>{item.title}</strong>
                  <p>
                    {item.contentText.length > 150
                      ? `${item.contentText.slice(0, 150)}…`
                      : item.contentText}
                  </p>
                  <small>
                    知乎{item.contentType} · {item.authorName} ·{" "}
                    {item.voteUpCount} 赞同 <b>查看全文摘要 →</b>
                  </small>
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div className="quote-paper">
              ...
              <p className="sentence-selection" onMouseUp={chooseSelection}>
                {sentences.map((item, index) => (
                  <span
                    data-quote-sentence
                    key={`${index}-${item}`}
                    className={sentence.includes(item) ? "selected" : ""}
                    onClick={() => setSentence(item)}
                  >
                    {item}
                  </span>
                ))}
            ...
            <button
              ...
              onClick={() =>
                onAdd(sentence, {
                  title: chosen.title,
                  author: chosen.authorName,
                  url: chosen.url,
                  contentId: chosen.id,
                  voteUpCount: chosen.voteUpCount,
                })
              }
            >
              确认入库
...
function PapersPage({
  ...
  const deletePaper = () => {
    ...
    onState((current) => ({
      ...current,
      papers: current.papers.filter((item) => item.id !== deletingPaperId),
    }));
  ...
  if (!paper)
    return (
      <main className="no-paper">
        <span className="empty-seal">卷</span>
        <h1>还没有人生答卷</h1>
        <p>回到笔记页，选中几条材料后提交。</p>
        <button className="tool primary" onClick={onNotes}>
          <Icon name="back" />
          去准备材料
        </button>
      </main>
  ...
  return (
    <main className="papers-page">
      <aside className="paper-sidebar">
        <div>
          <span className="eyebrow">答卷档案</span>
          <h2>
            {state.notebooks.find((item) => item.id === notebookId)?.name}
          </h2>
        </div>
        <div className="paper-list">
          {papers.map((item) => {
            ...
            return (
              <div
                key={item.id}
                className={`paper-entry ${item.id === paper.id ? "active" : ""}`}
              >
                <button
                  className="paper-select"
                  onClick={() => onPaper(item.id)}
                >
                  <span>{paperName}</span>
                  <small className={finished ? "finished" : "unfinished"}>
                    <b>{score}</b>
                    {finished ? "已答完" : "未答完"}
                  </small>
                </button>
                <button
                  ...
                  aria-label={`删除答卷 ${paperName}`}
                  onClick={() => setDeletingPaperId(item.id)}
                >
                  <Icon name="trash" />
                </button>
              </div>
        ...
        <button className="side-back" onClick={onNotes}>
          <Icon name="back" />
          继续补充笔记
        </button>
      </aside>
      <div className="paper-workspace">
        {notice && <div className="generation-notice">{notice}</div>}
        <div className="paper-toolbar">
          <span>
            完成得分 <b>{finishedScore}</b> / 100
          </span>
          <div>
            ...
            <button onClick={onRegenerate}>重新作答</button>
          </div>
        </div>
        <article className="exam-paper">
          ...
          <header className="exam-head">
            <div>
              <span className="paper-kicker">ZHIXING · LIFE PAPER</span>
              <h1>人生答卷</h1>
            </div>
            <div className="paper-number">
              ...
              {new Date(paper.createdAt)
                ...
                .slice(0, 10)
                .replaceAll("-", "")}
              -{String(paper.version).padStart(2, "0")}
            </div>
          </header>
          <div className="candidate">
            ...
            <span>
              日期：
              <i>{new Date(paper.createdAt).toLocaleDateString("zh-CN")}</i>
            </span>
            <span>
              卷次：<i>{chineseNumber(paper.version)}卷</i>
            </span>
          </div>
          <PaperSection
            number="一"
            title="题目"
            meta={`来源笔记 ${paper.noteIds.length} 条`}
          >
            <p className="question">{paper.question}</p>
          </PaperSection>
          <PaperSection number="二" title="已知条件" meta="你的现实边界">
            <ul>
              {paper.conditions.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </PaperSection>
          <PaperSection number="三" title="参考依据" meta="摘录与经验">
            <ul className="references">
              {paper.references.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ul>
          </PaperSection>
          <PaperSection number="四" title="解题步骤" meta="本题 100 分">
          ...
          <PaperSection number="五" title="得分点" meta="阶段检查">
            <ol className="checkpoints">
              {paper.checkpoints.map((item, index) => (
                <li key={item}>
          ...
          <PaperSection number="六" title="阅卷评语" meta="额外提醒">
            <blockquote>“{paper.comment}”</blockquote>
          </PaperSection>
          <footer className="exam-foot">
            <span>知行 · 第 {paper.version} 次作答</span>
            <span className="final-seal">知行</span>
            <span>完成比完美更重要</span>
          </footer>
        </article>
      </div>
      {deletingPaper && (
        <div
          className="overlay confirm-overlay"
          onMouseDown={(event) =>
            event.target === event.currentTarget && setDeletingPaperId(null)
          }
        >
          <section
            className="submit-confirm"
          ...
          >
            <span className="confirm-seal">删</span>
            <h2 id="delete-paper-title">
              删除“{chineseNumber(deletingPaper.version)}卷”？
            </h2>
            <p>
              这份答卷及其中记录的任务完成进度将被永久删除，且无法恢复；原始笔记不会受到影响。
            </p>
            <div>
              <button onClick={() => setDeletingPaperId(null)}>取消</button>
              <button className="danger-primary" onClick={deletePaper}>
                确认删除
              </button>
            </div>
          </section>
