...
export async function deleteNote(id: string, uid: string): Promise<void> {
  ...
  try {
    ...
    await client.query('delete from note n using notebook b where n.id = $1 and b.id=n.notebook_id and b.user_id=$2', [id, uid])
    await client.query('commit')
  } catch (error) { await client.query('rollback'); throw error } finally { client.release() }
}
export async function deletePaper(id: string, uid: string): Promise<void> { const db = requirePool(); await db.query('delete from paper p using notebook b where p.id = $1 and b.id=p.notebook_id and b.user_id=$2', [id, uid]) }

export async function createPaper(paper: Paper, uid: string): Promise<void> {
  const db = requirePool(); const client = await db.connect()
  try {
    await client.query('begin')
    const notebook = await client.query('select 1 from notebook where id=$1 and user_id=$2', [paper.notebookId, uid])
    if (!notebook.rows[0]) throw new Error('无权操作该答卷所属笔记本')
    if (paper.noteIds.length) {
      const notes = await client.query('select count(*)::int as count from note n join notebook b on b.id=n.notebook_id where n.id=any($1::uuid[]) and b.user_id=$2', [paper.noteIds, uid])
      if (notes.rows[0].count !== paper.noteIds.length) throw new Error('答卷包含无权访问的笔记')
    }
